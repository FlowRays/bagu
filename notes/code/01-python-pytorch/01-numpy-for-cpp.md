...
$$\boxed{\text{规约完还要和原数组运算，就一定要 keepdims=True}}$$
...
> **答：** **会。** `reshape` 和 `.T` 都**不复制数据**，只是换了一组 $(\text{shape},\text{strides})$ 去解释同一块 buffer。所以 `b` 和 `a` 共享内存，改一个另一个跟着变。
> 这和 C++ 的值语义相反：这里默认是**共享**，要真副本得写 `a.copy()`。

**2.** 写出广播的两条规则。`(3,4)` 和 `(3,)` 能不能相加？怎么改才行？

> **答：** **从右往左**逐维比较：① 长度相等 → OK；② 其中一个是 1 → 撑开到另一个的长度。否则报错。
> `(3,4) + (3,)`：`(3,)` 先补成 `(1,3)`，最后一维 4≠3，**报错**。
> 改法：如果本意是给每一行加一个数，写成 `(3,1)`：`a + b[:, None]`；如果是给每一列加，那 `b` 应该是长度 4 的 `(4,)`。

**3.** `keepdims=True` 什么时候必须加？举 LayerNorm 的例子。

> **答：** **规约完还要和原数组做运算时必须加**，否则维度对不上没法广播。
> ```python
> mu  = x.mean(-1, keepdims=True)   # (2,3,1)，能广播回 (2,3,4)
> var = x.var(-1, keepdims=True)
> out = gamma * (x - mu) / np.sqrt(var + eps) + beta
> ```
> 不加的话 `x.mean(-1)` 是 `(2,3)`，和 `(2,3,4)` 相减直接报错。

**4.** `np.maximum(a, b)` 和 `a.max()` 的区别？

> **答：** `np.maximum(a,b)` 是**逐元素**取两个数组对应位置的较大值，返回和输入同形状的数组（`np.maximum(a,0)` 就是 ReLU）。
> `a.max()` 是**规约**，把整个数组（或沿某个 axis）压成一个标量/低一维的数组。
> 名字太像，是最常见的笔误来源。

**5.** 用一行取出 `logp` 每行第 `labels[i]` 列。这在交叉熵里是干什么的？

> **答：** ```python
> logp[np.arange(N), labels]
> ```
> 两个整数数组同时索引 = **按坐标对取**，等价于 C++ 的 `for(i) out[i] = logp[i][labels[i]];`。
> 在交叉熵里就是**取出每个样本正确类别的 log 概率**，再取负求平均就是 loss。同样的写法也能写入：`dz2[np.arange(N), y] -= 1` 就是就地把 one-hot 减掉得到 $p-q$。

**6.** `np.repeat` 和 `np.tile` 的区别？GQA 里为什么必须用前者？

> **答：** `np.repeat([1,2,3], 2)` → `[1,1,2,2,3,3]`（**逐元素重复**）；`np.tile([1,2,3], 2)` → `[1,2,3,1,2,3]`（**整体重复**）。
> GQA 里 KV 有 $h_{kv}$ 组、要广播给 $h$ 个 query head，**相邻的 $g$ 个 query head 必须共享同一组 KV**，所以要 AABBCC 而不是 ABCABC。用错了 head 和 KV 组就对不上，而且**不报错**。
> （torch 里对应的是 `repeat_interleave`，注意 `torch.repeat` 反而等价于 `np.tile`。）

**7.** `-7 // 2` 在 Python 和 C++ 里分别是多少？

> **答：** Python：`-7 // 2 == -4`（**向下取整**）；C++：`-7 / 2 == -3`（**向零截断**）。
> 算 shape、分块、索引时如果涉及负数，这个差异会直接出 bug。另外 Python 的 `/` 永远返回 float，算维度要用 `//`。
...
> ```
> `x[:, None]` 是 `(n,1)`、`y[None, :]` 是 `(1,m)`，广播成 `(n,m)`。
> 这就是把双重循环翻译成「插维 + 广播」的标准套路。若 `x`、`y` 是向量组（`(n,d)` 和 `(m,d)`），欧氏距离平方就是 `((x[:,None,:]-y[None])**2).sum(-1)`。
